...
[GitHub Releases page](https://github.com/gleachkr/Lectic/releases),
extract it, and put `lectic` on your PATH.
...
the key in the same shell session where you're running Lectic. If you
set it in `.bashrc`, you may need to restart your terminal or run
`source ~/.bashrc`.
:::
...
`prompt` is the system prompt. Lectic picks a default provider and
model based on your API keys, so you don't need to specify them.
...
flags and
[Custom Subcommands](./automation/03_custom_subcommands.qmd).
...
Make sure tools are defined under the `tools` key inside
`interlocutor`, not at the top level:
...
1. **Set up your editor.** The intended workflow is to run Lectic with
   a single keypress. See
   [Editor Integration](./03_editor_integration.qmd) for Neovim,
