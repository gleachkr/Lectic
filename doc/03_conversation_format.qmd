...
[Configuration Reference](./04_configuration.qmd).
...
Lectic uses a syntax called "container directives" to represent messages from
the LLM. These are fenced blocks that start with three or more colons,
followed immediately by the name of the interlocutor.

The block is opened by `:::[InterlocutorName]` and closed by a matching
number of colons `:::`.
