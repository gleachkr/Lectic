...
3.  **`--Include` (`-I`) Flag**: You can specify an arbitrary YAML file to
    include using the `--Include` or `-I` command-line flag. This is handy
    for temporary or experimental settings.
    ```bash
    lectic -I project_settings.yaml -f my_convo.lec
    ```

4.  **Lectic File Header**: The YAML frontmatter within your `.lec` file is
...
- **Arrays (Lists)**: Merged based on the `name` attribute of their
  ...
  useful for managing lists of tools and interlocutors.
