...
- `plugin: NAME` searches recursively, in order, through:
  ...
  3. `LECTIC_DATA`.
- Plugin imports match a directory named `NAME` containing `lectic.yaml`.
- Imports are recursive, so imported files can have their own `imports`.
- Import cycles are detected and reported as errors.

Use `plugin:` when a plugin has already been installed into one of Lectic's
runtime/config/data roots and you want its bundled config. Use `path:` or a
string path for in-repo modules and other explicit filesystem imports.
