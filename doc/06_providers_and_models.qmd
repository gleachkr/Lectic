...
When `OPENAI_API_KEY` is detected, the auto-selected provider is `openai/chat`
(the legacy Chat Completions API), **not** `openai` (the Responses API). If you
want the Responses API, set `provider: openai` explicitly.
...
For API-key providers, `interlocutor.account` overrides the default key for
that interlocutor. The value may be a literal string or a `file:` / `exec:`
source.
