...
Lectic includes one built-in macro: `:cmd[...]`. It executes a shell
command and expands to the command's output wrapped in XML. See
[External Content](../context_management/01_external_content.qmd#command-output-via-cmd)
for details.

Because `:cmd` is a macro, it composes naturally with user-defined macros.
