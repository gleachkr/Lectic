...
`label_description` is shown inline in LSP completion UIs beside the inserted
`completion` text. This is useful when you want a short inserted value but a
more descriptive visible label.
...
When a macro expands via `exec`, the script being executed can be
passed information via environment variables.
