...
- `async`: (Optional) A boolean. If `true`, Lectic starts the hook in the
  background and does not wait for it to finish. Defaults to `false`.
  Async hooks are best-effort. They cannot inject inline output, and their
  eventual exit status cannot affect the current run.
...
If you set `async: true`, Lectic only waits long enough to start the hook
process. The hook then continues in the background. This is useful for
editor notifications, progress updates, and similar non-blocking side
effects.
