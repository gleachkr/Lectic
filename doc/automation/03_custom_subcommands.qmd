...
- `LECTIC_RUNTIME`: Optional PATH-style directory list used for recursive
  subcommand discovery. If set, it replaces the default recursive search
  roots (`LECTIC_CONFIG` and `LECTIC_DATA`).
...
1. `<recursive-root>/completions/` where recursive roots are:
   - `$LECTIC_RUNTIME` entries (if set), or
...
(`$LECTIC_RUNTIME` if set, otherwise `$LECTIC_CONFIG` and
