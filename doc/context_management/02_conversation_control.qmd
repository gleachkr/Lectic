...
When one interlocutor is shown an earlier turn from another interlocutor,
Lectic forwards the prose from that assistant block with the original
speaker name attached. Structured transcript records such as `<tool-call>`,
`<thought-block>`, and `<inline-attachment>` are omitted from this
cross-interlocutor view.
