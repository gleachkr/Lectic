...
You can apply sandboxes project-wide (top-level), per-interlocutor, or per-
tool.
...
**Project-wide default (recommended for per-project configs):**
This wraps all `exec` tools and local `mcp_command` tools in the project,
...
**Per-interlocutor default:**
Useful when different interlocutors need different isolation.
...
- **Environment variables**: Lectic passes variables like
  ...
  [Configuration Reference](../05_configuration.qmd#overriding-default-directories).
...
- **Quoting and arguments**: A sandbox is a command string. Single-line
  ...
  `exec` commands. If a path contains spaces, quote that path. If you need
  complex quoting or structured options, prefer writing a wrapper script.
