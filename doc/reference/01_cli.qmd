...
- `lectic script`
  Run an ES module file as a Lectic subcommand.

  You can export a default function, which will be called with no arguments;
  script arguments can be called via `process.argv`.

  Usage:
  ```bash
  lectic script <module-path> [args...]
  ```
