...
- `-i`, `--inplace`
  Update the file in place. Requires `--file`.

- `--format <mode>`
  Controls output shape and visibility. Supported modes:
  ...
  - `clean`: stream only assistant text, stripping tool calls, thought
    blocks, and inline attachments. This is similar to the prose-only view
    used when replaying one interlocutor's prior assistant turn to another.
  - `none`: print nothing

- `-s`, `--short` (deprecated)
  Alias for `--format block`.
...
- --format cannot be combined with legacy output flags:
  --short, --Short, or --quiet.
