...
- `name`: (Optional) A name for the hook. Used for merging and overriding
  hooks from different configuration sources. For inline hooks, this is also
  serialized as a `name` attribute on `<inline-attachment kind="hook">`
  blocks and shown in LSP fold text.
