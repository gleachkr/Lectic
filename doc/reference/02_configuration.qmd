...
- `sandbox`: A path to a sandboxing script. If present, this script wraps
  execution for all `exec` tools and local `mcp_command` tools used by this
  interlocutor, unless overridden by the tool's own `sandbox` setting.
...
- `sandbox`: Path to a sandboxing script that wraps execution.
...
- `sandbox`: Optional wrapper script to isolate `mcp_command` servers.
