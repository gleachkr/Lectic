...
- `on`: (Required) A single event name or a list of event names to trigger
  the hook. Supported events are `user_message`, `assistant_message`,
  `assistant_final`, `assistant_intermediate`, `tool_use_pre`,
  `tool_use_post`, `run_start`, `run_end`, and `error`.
