...
- Place the cursor on a directive (e.g., `:name[]`, `:ask[Name]`) or
  a name field in the YAML header (e.g., `name: Assistant`) and invoke
  “Go to Definition” to jump to the relevant definition. If multiple
  definitions exist (e.g., a local override of a workspace or system
  interlocutor), the LSP returns all locations, prioritized by
  proximity (local > workspace > system).
