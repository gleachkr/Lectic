...
- Features: `textDocument/completion`, diagnostics,
  document symbols (outline), folding ranges
...
- Triggers: `:` and `[` (the server filters so only `:ask[`/`:aside[`/
  `:macro[` actually produce bracket completions)
...
- The folded range includes both the opening and closing tags. The
  entire block is concealed when folded.
- Fold ranges are not produced inside fenced code blocks. Markdown
  fences are skipped, so literal `<tool-call>` text inside a fence does
  not fold.
...
Notes
- The server does not run hovers.
