...
- YAML header fields:
  - `interlocutor` / `interlocutors` blocks: top‑level interlocutor
    ...
    `thinking_budget`, `reminder`, `tools`, and `nocache`.
...
- Troubleshooting
  ...
  - Directive completion only triggers on a single `:` (not `::` or
    `:::`). Tool‑kind completion triggers on the first line of a tools
    item after `-`.
