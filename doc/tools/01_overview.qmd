...
If you reuse the same set of tools across interlocutors, define a named
kit once and reference it by name in each interlocutor. Kits are
listed at the top level of the header.
...
You can still mix regular tools alongside kit references.

## Hooks

Tool calls will trigger `tool_use_pre` hook, which fire after the parameters
have been supplied to the tool, but before the tool returns results. If the
hook exits with non-zero exit status, the tool call will be blocked and the LLM
will receive a "permission denied" error. For details see
[Hooks](../automation/02_hooks.qmd). If you want to have a `tool_use_pre` hook
that applies only to a certain tool, you can provided it as part of the YAML
definition of the tool, writing for example:

```yaml
---
interlocutor:
  tools:
    - exec: clean_directory.sh
...
Note that `native` tools do not support hooks.
