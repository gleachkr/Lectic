...
interlocutor with a valid prompt and model configuration. See
[Getting Started](../02_getting_started.qmd) for a full header
example.
...
- `boilerplate`: Whether to include the standard exec-tool description before
  the `usage` string. If `false`, then `usage` must be a complete standalone
  description. If `true` (the default), Lectic prepends the standard exec
  guidance and then appends `usage`.
- `usage`: A string with instructions for the LLM. It also accepts `file:`
  ...
  for semantics.
- `sandbox`: A command string to wrap execution (e.g. `/path/to/script.sh`
  or `wrapper.sh arg1`). See safety below. Overrides any interlocutor-level
  or top-level sandbox.
- `timeoutSeconds`: Seconds to wait before aborting a long‑running call.
- `limit`: Maximum number of output characters returned across stdout and
  stderr. Output beyond this is truncated. Default: `100000`.
...
- Single-line commands are split into argv using simple shell-like rules:
  single '...' and double "..." quotes are supported; no globbing or
  substitution.
- After tokenization, environment variables are expanded within each token
  using the tool's `env` plus standard Lectic variables. This preserves
  quoted token boundaries even when an expanded variable contains spaces.
...
It also always includes the numeric exit code. You will see these
serialized inside the tool call results as XML tags like <stdout>,
<stderr>, and <exitCode>.
...
Each parameter can be either a string describing the parameter, or a complete
JSON schema specifying it more precisely. Lectic exposes it to the subprocess
via an environment variable with the same name. Non-string values returned by
the model are stringified as JSON.
...
When Lectic runs your command or script, it sets the following environment
variables in addition to any you configure via the `env` field:
...
You can use the `tool_use_pre` hook to implement confirmation dialogs or logic.
See [Hooks](../automation/02_hooks.qmd) for examples.
...
provided parameters as arguments. The wrapper is responsible for creating a
controlled environment to run the command.
...
expands environment variables within each token. You can include arguments
in the sandbox string (for example, `bwrap.sh --net`). If a sandbox path
contains spaces, quote that path so it stays a single argv element.

See the [Custom Sandboxing](../cookbook/06_custom_sandboxing.qmd) cookbook
recipe for a detailed guide on writing your own sandbox scripts.
...
`interlocutor` object. If set, it applies to all `exec` tools that don't
specify their own. Tool-level `sandbox` wins over both defaults.
