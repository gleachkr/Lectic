...
- `text` (required): user text to send to the agent.
- `contextId` (optional): A2A context id to continue a conversation.
  - If omitted, the agent starts a new conversation.
  - If explicitly `null`, the tool omits the field (same effect as omit).
- `taskId` (optional): A2A task id (rarely needed).
- `stream` (optional): override streaming for this call.

## Output format

The tool returns two results:

1. `text/plain`: the agent response text
2. `application/json`: call metadata
