...
describe("A2A server monitoring endpoints", () => {
  test("legacy agent-scoped monitor endpoints still work", async () => {
    const runtime = {
    ...
    agents.set(agentId, { agentId, handler, card: mkCard(), transport })
    ...
    try {
      const t1 = (await handler.sendMessage(
        mkSendParams({ text: "hi", contextId: "c" })
      )) as Task

      await waitForTerminal(handler, t1.id)

      const base = `http://127.0.0.1:${server.port}/agents/${agentId}`
      ...
      const ids = (list.tasks ?? []).map((t) => t.taskId)
  ...
  test("legacy agent-scoped monitor/events emits task lifecycle events", async () => {
    const runtime = {
      interlocutorName: "Agent",
      runTurn: async (args: {
        onAssistantPassText?: (text: string) => void
    ...
    try {
      ...
      void handler.sendMessage(mkSendParams({ text: "hi", contextId: "c" }))
      ...
      for (let i = 0; i < 10; i++) {
        ...
        if (ev.kind === "event" && ev.event?.kind === "created") {
          gotCreated = true
          break
        }
      }
